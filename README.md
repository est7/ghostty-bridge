...
| `ghostty-bridge read <target> [lines]` | Read terminal output (last N lines, default 50) |
...
```sh
# Open a new tab and type a command into the shell (inherits full shell env)
ghostty-bridge open tab --cwd ~/src/project --input "cargo test" --label test
...
`--input` types text into the shell after it starts, so the full shell profile (PATH, etc.) is available. `--command` replaces the shell process via exec — use it only for standalone binaries that don't need your shell environment.
